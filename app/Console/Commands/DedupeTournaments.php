...
class DedupeTournaments extends Command
{
    ...
    public function handle(): int
    {
        ...
        foreach (Tournament::whereNotNull('external_id')->orderBy('id')->get() as $synced) {
            ...
            // One merge is all-or-nothing: re-pointing plans/results, deleting
            // the dupe, and moving its identity onto the curated row must not
            // half-apply if a step fails midway.
            DB::transaction(function () use ($synced, $curated, $identity) {
