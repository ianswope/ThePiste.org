...
- **Event prep tracker** (`/season/prep`) — Livewire `PrepTracker`. Per planned event, a readiness checklist (X/5): Registered (reuses plan-item `status`), Fees paid (reuses `paid`), Travel & Lodging (`travel_status`/`lodging_status`: pending|booked|na), Coaching (`coaching_status`: undecided|arranged|none) — `PlanItem::prepChecklist()`/`prepProgress()`. Each card also has per-event "Add to calendar" (Google template link via `Tournament::googleCalendarUrl()`, and Apple/`.ics` via `/events/{tournament}.ics`). Skipped events drop off. Marking Registered also stands down the registration-reminder email (those target `status=planned`).
- **Calendar export** — iCal everywhere via `App\Support\Ics` (shared builder). Whole-plan feed at `/p/{slug}.ics` (download + webcal subscribe on the share page); single events at `/events/{tournament}.ics` (public; catalog facts). All all-day, end-exclusive.
...
- **Email digests**: `thepiste:notify-new-events` (daily 06:30, after the sync) mails each user the newly-cataloged events relevant to their fencers (TierService tiers nac/home/priority/drive, or any goal contribution); idempotent via `tournaments.alerted_at`. `thepiste:send-registration-reminders` (daily 07:00) nudges `planned` plan items entering their registration window; AskFRED has no true deadlines, so lead times encode norms (`config/fencing.php` `reminder_lead_days`: nationals/FIE 45 days, default 14); idempotent via `plan_items.reminded_at`. Both support `--dry-run`. The digest notifications (`App\Notifications\FencerDigest` subclasses) are `ShouldQueue`, so delivery runs on the queue with retries, not inline in the command.
- **Queue worker**: `QUEUE_CONNECTION=database` (jobs/failed_jobs tables; Redis is up but unused here). Supervisor runs `thepiste-worker` (`/etc/supervisor/conf.d/thepiste-worker.conf`, 1 proc, `queue:work`, log `storage/logs/worker.log`); the box's supervisor also runs escrimepro's worker. `deploy.sh` ends with `php artisan queue:restart` so the worker reloads new code. Check failures: `php artisan queue:failed`; retry: `queue:retry all`.
